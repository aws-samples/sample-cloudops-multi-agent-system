...
from agents.shared.registry import (
    _extract_cfn_artifacts,
    _pending_cfn_artifacts,
    get_pending_cfn_artifacts,
)
...
class TestUnterminatedArtifactFallback:
    def test_no_close_tag_flushes_partial_with_marker(self):
        ...
        joined = "".join(flushed)
        assert "partial-yaml-content" in joined
        assert "unterminated" in joined.lower() or "partial" in joined.lower()
...
class TestAbbreviateBulkyToolTraces:
    ...
    def test_abbreviated_turn_stays_under_memory_limit(self):
        ...
        from agents.shared.memory import build_enriched_text, _scrub_cfn_artifacts
        ...
        saved = _scrub_cfn_artifacts(build_enriched_text(segments))
...
# ---------------------------------------------------------------------------
# 10. _extract_cfn_artifacts — primary path (tool-result level extraction)
# ---------------------------------------------------------------------------


class TestExtractCfnArtifacts:
    """The primary delivery path: _delegate extracts <cfn-artifact> blocks
    from the sub-agent response before the supervisor model sees them.
    """

    def test_no_artifact_returns_unchanged(self):
        data = "Summary text with no artifact tags."
        sanitized, artifacts = _extract_cfn_artifacts(data)
        assert sanitized == data
        assert artifacts == []

    def test_single_artifact_extracted(self):
        data = (
            "Summary.\n"
            '<cfn-artifact title="My alarms">YAML body here</cfn-artifact>'
            "\nApply it."
        )
        sanitized, artifacts = _extract_cfn_artifacts(data)
        assert "YAML body here" not in sanitized
        assert "[CloudFormation template generated: My alarms]" in sanitized
        assert "Summary." in sanitized
        assert "Apply it." in sanitized
        assert len(artifacts) == 1
        assert artifacts[0]["title"] == "My alarms"
        assert artifacts[0]["yaml"] == "YAML body here"

    def test_multiple_artifacts_extracted(self):
        data = (
            '<cfn-artifact title="a">YAML A</cfn-artifact>'
            " middle "
            '<cfn-artifact title="b">YAML B</cfn-artifact>'
        ...
        assert artifacts[0] == {"title": "a", "yaml": "YAML A"}
        assert artifacts[1] == {"title": "b", "yaml": "YAML B"}
        assert "YAML A" not in sanitized
        assert "YAML B" not in sanitized
        assert " middle " in sanitized

    def test_multiline_yaml_body(self):
        yaml_body = "AWSTemplateFormatVersion: '2010-09-09'\nResources:\n  A: {}\n"
        data = f'<cfn-artifact title="test">{yaml_body}</cfn-artifact>'
        sanitized, artifacts = _extract_cfn_artifacts(data)
        assert artifacts[0]["yaml"] == yaml_body
        assert "AWSTemplateFormatVersion" not in sanitized

    def test_missing_title_defaults(self):
        data = "<cfn-artifact>YAML</cfn-artifact>"
        _, artifacts = _extract_cfn_artifacts(data)
        assert artifacts[0]["title"] == "CloudFormation alarm template"

    def test_large_yaml_does_not_reach_model(self):
        big_yaml = "x" * 100_000
        data = f'<cfn-artifact title="big">{big_yaml}</cfn-artifact>'
        sanitized, artifacts = _extract_cfn_artifacts(data)
        assert len(sanitized) < 200
        assert len(artifacts[0]["yaml"]) == 100_000


# ---------------------------------------------------------------------------
# 11. _handle_cfn_artifacts_from_tool_result — event-loop integration
# ---------------------------------------------------------------------------
...
class TestHandleCfnArtifactsFromToolResult:
    ...
    def test_artifacts_present_emits_report_pending(self):
        ...
        reg._pending_cfn_artifacts.append(
            {"title": "Test", "yaml": "AWSTemplateFormatVersion: '2010-09-09'"}
        )
        event = SimpleNamespace(type="TOOL_CALL_RESULT", content="{}")
    ...
    def test_persistence_failure_falls_back_to_yaml(self):
        import agents.shared.registry as reg
        from types import SimpleNamespace

        reg._pending_cfn_artifacts.append(
            {"title": "X", "yaml": "Resources: {}"}
        )
        event = SimpleNamespace(type="TOOL_CALL_RESULT", content="{}")
        ...
        assert len(events) == 1
        delta = getattr(events[0], "delta", "")
        assert "```yaml" in delta
        assert "Resources: {}" in delta
