...
def build_cfn_alarm(
    ...
    alarm_name = f"{metric_name}-{dim_hash}" if metric_name else f"alarm-{dim_hash}"
...
def assemble_cfn_template(
    ...
    This is the single source of the template artifact payload. The worker
    captures the returned structured value before it reaches a model response
    or tool trace, so this function must return the complete deterministic YAML
    without embedding any presentation markup.
