...
- **Frontend production builds use Webpack intentionally.** Next.js 16 Turbopack currently panics while processing `driver.js/dist/driver.css`, so `src/frontend/package.json` keeps `next build --webpack`. Re-test before removing that flag after upgrading Next.js or `driver.js`.
